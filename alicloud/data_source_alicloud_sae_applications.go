...
func dataSourceAlicloudSaeApplicationsRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_sae_applications", action, AlibabaCloudSdkGoERROR)
		}
	...
	for _, object := range objects {
		...
		mapping["config_map_mount_desc"] = getResp["ConfigMapMountDesc"]
		...
		if statusOk && status != "" && status != getResp2["CurrentStatus"].(string) {
			continue
		}
		mapping["create_time"] = getResp2["CreateTime"]
		mapping["status"] = getResp2["CurrentStatus"]
